...
class CallContractTransactionV5Spec extends PropSpec with ScalaCheckPropertyChecks with Matchers with ContractTransactionGen {

  property("CallContractTransactionV5Spec serialization roundtrip") {
    forAll(callContractV5ParamGen()) { tx =>
      val recovered = CallContractTransactionV5.parseBytes(tx.bytes()).get
      recovered shouldBe tx
    }
  }

  property("CallContractTransactionV5 proto serialization roundtrip") {
    forAll(callContractV5ParamGen()) { tx =>
      val recovered = CallContractTransactionV5.fromProto(tx.toInnerProto).explicitGet()
      recovered shouldBe tx
    }
  }

  property("CallContractTransactionV5Spec serialization from TypedTransaction") {
    forAll(callContractV5ParamGen()) { tx =>
