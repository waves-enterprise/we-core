...
class ExecutedContractTransactionV4Spec
    extends PropSpec
  ...
  property("ExecutedContractTransactionV4 serialization roundtrip") {
    forAll(executedContractV4ParamGen) { tx: ExecutedContractTransactionV4 =>
      val recovered = ExecutedContractTransactionV4.parseBytes(tx.bytes()).get
      recovered shouldBe tx
    }
    forAll(bigExecutedContractV4ParamGen) { tx: ExecutedContractTransactionV4 =>
      val recovered = ExecutedContractTransactionV4.parseBytes(tx.bytes()).get
      recovered shouldBe tx
    }
  }

  property("ExecutedContractTransactionV4 proto serialization roundtrip") {
    forAll(executedContractV4ParamGen) { tx =>
      val recovered = ExecutedContractTransactionV4.fromProto(tx.toInnerProto).explicitGet()
      recovered shouldBe tx
    }
    forAll(bigExecutedContractV4ParamGen) { tx =>
      val recovered = ExecutedContractTransactionV4.fromProto(tx.toInnerProto).explicitGet()
      recovered shouldBe tx
    }
  }

  property("ExecutedContractTransactionV4 serialization from TypedTransaction") {
    forAll(executedContractV4ParamGen) { tx: ExecutedContractTransactionV4 =>
  ...
  property("JSON format validation with CreateContractTransactionV1") {
    ...
    val execTx = ExecutedContractTransactionV4
      .create(
        sender = PublicKeyAccount(senderAccount.publicKey),
        tx = tx,
