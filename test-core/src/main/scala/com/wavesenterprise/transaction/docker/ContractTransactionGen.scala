...
trait ContractTransactionGen extends CommonGen with WithSenderAndRecipient { _: Suite =>
  ...
  val createContractV1ParamGen: Gen[CreateContractTransactionV1] =
    ...
    } yield CreateContractTransactionV1
  ...
  def createContractV2ParamGen(optAssetIdGen: Gen[Option[AssetId]],
    ...
    for {
      ...
      feeAmount    <- amountGen
      timestamp    <- ntpTimestampGen
    } yield CreateContractTransactionV2
  ...
  def createContractV3ParamGen(atomicBadgeGen: Gen[Option[AtomicBadge]],
    ...
    for {
      ...
      timestamp    <- ntpTimestampGen
    } yield CreateContractTransactionV3
      .selfSigned(signer, new String(image, UTF_8), imageHash, new String(contractName, UTF_8), params, feeAmount, timestamp, optAssetId, atomicBadge)
      .explicitGet()
  ...
  def createContractV4ParamGen(atomicBadgeGen: Gen[Option[AtomicBadge]],
    ...
    for {
      ...
      timestamp          <- ntpTimestampGen
    } yield CreateContractTransactionV4
      .selfSigned(
        signer,
  ...
  val createContractV7ParamGen: Gen[CreateContractTransactionV7] =
    for {
      ...
      .selfSigned(
        ...
        groupOwners.toSet,
        WasmContract(bytecode, bytecodeHash)
      ).explicitGet()
  ...
  } yield CreateContractTransactionV2.selfSigned(signer, tx.image, tx.imageHash, tx.contractName, params, tx.fee, tx.timestamp, tx.feeAssetId).explicitGet()
  ...
  def executedContractV5ParamGen(otherCalls: Int = 2): Gen[ExecutedContractTransactionV5] =
    for {
      sender <- accountGen
      tx <- Gen.oneOf(
        createContractV7ParamGen,
        callContractV7ParamGen()
      )
      callTransactions <- Gen.listOfN(otherCalls, callContractV7ParamGen())
      assetOperations  <- listContractAssetOperationV1Gen(tx.id(), Gen.choose(0, 20), accountOrAliasGen, genOptAssetId, issueNonceGen)
      innerOperations = callTransactions.map(tx =>
        tx.contractId -> listContractAssetOperationV1Gen(
          tx.contractId,
          Gen.choose(0, 20),
          accountOrAliasGen,
          genOptAssetId,
      ...
      results <- Gen
        .choose(ResultsMinCount, ResultsMaxCount)
        .flatMap(Gen.listOfN(_, dataEntryGen(DataEntryMaxSize)))
      innerResults = innerOperations.map(_._1 -> Gen
        .choose(ResultsMinCount, ResultsMaxCount)
        .flatMap(Gen.listOfN(_, dataEntryGen(DataEntryMaxSize))).sample.get)
      assetsMap   = ContractAssetOperationMap((innerOperations :+ (tx.contractId -> assetOperations)).toMap)
      resultsMap  = DataEntryMap((innerResults :+ (tx.contractId -> results)).toMap)
      resultsHash = ContractTransactionValidation.resultsMapHash(resultsMap, assetsMap)
      validationProofs <- validationProofsGen(resultsHash)
      timestamp        <- ntpTimestampGen
      outputCommitment <- commitmentGen()
      readingsHash     <- digestSizeByteStrGen.map(ReadingsHash.apply)
      readings         <- readingsGen
    } yield ExecutedContractTransactionV5.selfSigned(
      sender,
      tx,
      resultsMap,
      resultsHash,
  ...
  val specificKeysGen: Gen[SpecificKeys] = for {
    ...
    keys       <- Gen.nonEmptyListOf(Gen.asciiStr.map(_.take(10))).map(_.take(3))
  } yield SpecificKeys(contractId, keys)
  val filteredKeysGen: Gen[FilteredKeys] = for {
    contractId <- bytes32gen.map(ByteStr.apply)
    matches    <- Gen.option(Gen.asciiStr.map(_.take(10)))
  ...
  def callContractV3ParamGen(optAssetIdGen: Gen[Option[AssetId]], amountGen: Gen[Long]): Gen[CallContractTransactionV3] =
    for {
      ...
      feeAmount       <- amountGen
    } yield CallContractTransactionV3.selfSigned(signer,
                                                 tx.contractId,
                                                 tx.params,
                                                 feeAmount,
                                                 tx.timestamp,
                                                 contractVersion,
                                                 optFeeAssetId).explicitGet()
  ...
  def callContractV4ParamGen(atomicBadgeGen: Gen[Option[AtomicBadge]],
    ...
    for {
      ...
      feeAmount       <- amountGen
      atomicBadge     <- atomicBadgeGen
    } yield CallContractTransactionV4
  ...
  def callContractV5ParamGen(optAssetIdGen: Gen[Option[AssetId]],
    ...
    } yield CallContractTransactionV5
  ...
  def updateContractV2ParamGen(optAssetIdGen: Gen[Option[AssetId]], amountGen: Gen[Long]): Gen[UpdateContractTransactionV2] =
    for {
      ...
      optFeeAssetId <- optAssetIdGen
      feeAmount     <- amountGen
    } yield UpdateContractTransactionV2
  ...
  def updateContractV3ParamGen(atomicBadgeGen: Gen[Option[AtomicBadge]],
    ...
    for {
      ...
      optFeeAssetId <- optAssetIdGen
      feeAmount     <- amountGen
    } yield UpdateContractTransactionV3
  ...
  def updateContractV4ParamGen(atomicBadgeGen: Gen[Option[AtomicBadge]],
    ...
    for {
      ...
      contractApiVersion <- contractApiVersionGen
      timestamp          <- ntpTimestampGen
    } yield UpdateContractTransactionV4
      .selfSigned(signer,
                  contractId,
                  new String(image, UTF_8),
                  imageHash,
  ...
  def contractIssueV1Gen(nonceGen: Gen[Byte] = issueNonceGen,
    ...
    } yield ContractAssetOperation.ContractIssueV1(ByteStr(assetId),
      ...
      case _ =>
        ...
        newContractAssetOperation match {
          ...
          case _: ContractAssetOperation.ContractCancelLeaseV1 =>
            contractOperationsGen(leftOperations - 1, issuedAssetIds, result :+ newContractAssetOperation)
          case _: ContractAssetOperation.ContractPaymentV1 =>
            contractOperationsGen(leftOperations - 1, issuedAssetIds, result :+ newContractAssetOperation)
        }
    }
  ...
  def genContractIssueWithUniqueNonces(txId: ByteStr, issuedNonces: Set[Byte] = Set.empty): Gen[ContractAssetOperation.ContractIssueV1] = {
    ...
    } yield ContractAssetOperation.ContractIssueV1(assetId,
                                                   new String(name, UTF_8),
                                                   new String(description, UTF_8),
                                                   Long.MaxValue / 3,
                                                   decimals,
    ...
    def genContractOperations(leftOperations: Int,
      ...
      case _ =>
        ...
        nextOperation.retryUntil(o => o.isInstanceOf[ContractAssetOperation], 5).sample.get match {
          ...
          case contractPayment: ContractAssetOperation.ContractPaymentV1 =>
            genContractOperations(
              leftOperations - 1,
              availableNoncesState,
              resultList :+ contractPayment
            )
        }
    }
